...
export default async function handler(req) {
  ...
  const totalCorrect = testResults.reduce(function(s, tr) { return s + tr.answers.filter(function(a) { return a.correct; }).length; }, 0);
  const totalPossible = testResults.reduce(function(s, tr) { return s + tr.answers.length; }, 0);
  ...
  const topicRows = testResults.map(function(tr) {
    const correct = tr.answers.filter(function(a) { return a.correct; }).length;
    const pct = Math.round((correct / tr.answers.length) * 100);
  ...
  const breakdownHtml = testResults.map(function(tr) {
    const correct = tr.answers.filter(function(a) { return a.correct; }).length;
    const rows = tr.answers.map(function(a, i) {
      const res = a.correct ? '<span style="color:#1C7A47;font-weight:700;">PASS</span>' : '<span style="color:#B83232;font-weight:700;">FAIL</span>';
      const expl = !a.correct ? (a.explanation || '') : '';
      return '<tr style="border-bottom:1px solid #f5f5f5;"><td style="padding:8px 12px;font-size:12px;color:#6b7280;white-space:nowrap;">Q' + (i+1) + ' Gr' + a.grade + '</td><td style="padding:8px 12px;font-size:13px;">' + a.text + '</td><td style="padding:8px 12px;font-size:13px;text-align:center;">' + res + '</td><td style="padding:8px 12px;font-size:12px;color:#6b7280;">' + expl + '</td></tr>';
    }).join('');
  ...
  try {
    const emailResp = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to: TO_EMAIL,
        subject: 'Awad Maths Diagnostic - ' + studentName + ' (' + (yearGroup||'Year TBC') + ') - ' + totalCorrect + '/' + totalPossible + ' (' + overallPct + '%)',
        html: emailHtml
      })
    });
    if (!emailResp.ok) {
